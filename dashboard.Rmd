...
pal = colorNumeric(palette = "YlGnBu", domain = country_coordinates$scores, na.color = "#808080")
...
temp_df = temp_df %>% 
  ...
  mutate(color = pal(scores))

wmap$color = temp_df$color
wmap$scores = temp_df$scores
wmap$labels = paste(wmap$names, paste(":", ifelse(
  is.na(wmap$scores), "", formatC(wmap$scores, big.mark = ",", format = "f", digits = 0)
)), sep = "\n")
